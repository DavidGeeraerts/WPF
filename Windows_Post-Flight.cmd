...
SET PROCESS_6_FILE_NAME=Ran_Windows_Ultimate.txt
...
SET VAR_TLT_SEC=0
SET VAR_TLT_MIN=0
IF EXIST %LOG_LOCATION%\var\var_LAPSED_TIME.txt DEL /Q /F %LOG_LOCATION%\var\var_LAPSED_TIME.txt
FOR /F "tokens=6 delims= " %%P IN ('find /I "Time Lapsed" "%LOG_LOCATION%\%LOG_FILE%"') DO ECHO %%P >> %LOG_LOCATION%\var\var_LAPSED_TIME.txt
IF EXIST "%LOG_LOCATION%\var\var_LAPSED_TIME.txt" IF %LOG_LEVEL_DEBUG% EQU 1 ECHO %ISO_DATE% %TIME% [DEBUG]	File {var_LAPSED_TIME.txt} just got created! >> %LOG_LOCATION%\%LOG_FILE%
FOR /F "tokens=2 delims=:." %%P IN ('type "%LOG_LOCATION%\var\VAR_LAPSED_TIME.txt"') DO SET /a VAR_TLT_SEC+=%%P
...
FOR /F "tokens=1 delims=:." %%P IN ('type "%LOG_LOCATION%\var\var_LAPSED_TIME.txt"') DO SET /a VAR_TLT_MIN+=%%P
...
IF EXIST "%LOG_LOCATION%\RUNNING_%SCRIPT_NAME%_%SCRIPT_VERSION%.txt" DEL /F /Q "%LOG_LOCATION%\RUNNING_%SCRIPT_NAME%_%SCRIPT_VERSION%.txt"
