...
::	 though this can be done with powershell it requires text file output to properly parse, and output is not yet established;
::		could use a temp location
...
IF %DOMAIN_USER_STATUS% EQU 1 whoami /FQDN > %LOG_LOCATION%\var_whoami_fqdn.txt
SET /P var_WHOAMI_FQDN= < %LOG_LOCATION%\var_whoami_fqdn.txt
IF EXIST %LOG_LOCATION%\var_whoami_fqdn.txt IF %LOG_LEVEL_DEBUG% EQU 1 ECHO %ISO_DATE% %TIME% [DEBUG]	FQDN: %var_WHOAMI_FQDN% >> %LOG_LOCATION%\%LOG_FILE%
...
IF EXIST %LOG_LOCATION%\var_CONSOLE_USER.txt (IF %LOG_LEVEL_DEBUG% EQU 1 ECHO %ISO_DATE% %TIME% [DEBUG]	File {var_CONSOLE_USER.txt} just got created!) >> %LOG_LOCATION%\%LOG_FILE% 
...
IF NOT EXIST %LOG_LOCATION%\var_NETDOM_INSTALL.txt (IF %LOG_LEVEL_ERROR% EQU 1 ECHO %ISO_DATE% %TIME% [ERROR]	An error occured looking for RSAT-Remote Server Administrative Tools [NETDOM] installer!) >> %LOG_LOCATION%\%LOG_FILE%
...
IF EXIST %LOG_LOCATION%\%PROCESS_COMPLETE_FILE% (
	IF EXIST %LOG_LOCATION%\WPF_SHA256_check.txtt DEL /F /Q %LOG_LOCATION%\WPF_SHA256_check.txt)
IF NOT EXIST %LOG_LOCATION%\WPF_SHA256_check.txt (
     IF %LOG_LEVEL_DEBUG% EQU 1 ECHO %ISO_DATE% %TIME% [DEBUG]	File {WPF_SHA256_check.txt} deleted!) >> %LOG_LOCATION%\%LOG_FILE%
