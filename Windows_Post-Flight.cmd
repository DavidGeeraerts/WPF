...
(NETDOM HELP 2> nul) && (SET NETDOM_PRESENCE=1)
IF %NETDOM_PRESENCE% EQU 1 DISM /online /Get-CapabilityInfo /CapabilityName:Rsat.ActiveDirectory.DS-LDS.Tools~~~~0.0.1.0 > %LOG_LOCATION%\var\var_DISM_RSAT.txt
...
:: LOG SHIPPING
IF EXIST %LOG_LOCATION%\%PROCESS_COMPLETE_FILE% IF %DOMAIN_USER_STATUS% EQU 1 IF DEFINED LOG_SHIPPING_LOCATION IF NOT EXIST "%LOG_SHIPPING_LOCATION%" MD "%LOG_SHIPPING_LOCATION%"
IF EXIST %LOG_LOCATION%\%PROCESS_COMPLETE_FILE% IF %DOMAIN_USER_STATUS% EQU 1 IF DEFINED LOG_SHIPPING_LOCATION (
...
IF EXIST %LOG_LOCATION%\INCOMPLETE_%SCRIPT_NAME%_%SCRIPT_VERSION%.log IF %DOMAIN_USER_STATUS% EQU 1 IF DEFINED LOG_SHIPPING_LOCATION (
	IF EXIST "%LOG_SHIPPING_LOCATION%\%LOG_FILE%" (Type %LOG_LOCATION%\%LOG_FILE% >> "%LOG_SHIPPING_LOCATION%\%LOG_FILE%")) ELSE (
	COPY /Y "%LOG_LOCATION%\%LOG_FILE%" "%LOG_SHIPPING_LOCATION%"
	)
:: Console logoff
