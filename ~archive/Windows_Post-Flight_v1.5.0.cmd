...
SET WPF_CONFIG_SCHEMA_VERSION_MIN=1.4.2
...
::   WIRELESS_CONFIG
FOR /F "tokens=2 delims=^=" %%V IN ('FINDSTR /BC:"WIRELESS_CONFIG" "%~dp0\%CONFIG_FILE_NAME%"') DO SET "WIRELESS_CONFIG=%%V"
...
ECHO %TIME% [DEBUG]	VARIABLE: WIRELESS_CONFIG set to %WIRELESS_CONFIG% >> %LOG_LOCATION%\%LOG_FILE%
...
IF %NETDOM_PRESENCE% EQU 0 (IF %LOG_LEVEL_WARN% EQU 1 ECHO %TIME% [WARN]	NETDOM is NOT present!) >> %LOG_LOCATION%\%LOG_FILE%
